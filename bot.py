...
def ensure_user(update):
    ...
    if not user:
        ...
        users_col.update_one(
            {"_id": u_id},
            {"$addToSet": {"chats": c_id}, "$set": {"full_name": update.effective_user.full_name}}
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✨ **Вітаємо в Kapyland!** ✨\n\n{random.choice(ORIGIN_STORIES)}",
        parse_mode="Markdown"
    )
...
async def feed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Четверта стіна" in user["blessings"]:
    ...
    if "Повільний метаболізм" in user["blessings"]:
        gain *= 1.5

    if "Фібоначчі" in user["blessings"]:
        gain += user["history"][-1]

    if "Вічна фієста" not in user["blessings"]:
        r = random.random()
        if r < 0.4:
            gain = -gain
    ...
    if "Дієта" in user["curses"]:
        gain = 0

    if "Лудоман" in user["curses"]:
        gain += random.uniform(-10, 10)

    new_weight = max(1.0, round(user["weight"] + gain, 2))
    ...
    await update.message.reply_text(
        f"{log}🍊 Приріст: **{round(gain,2)}кг**\n⚖️ Вага: {sanitize_weight(new_weight, user['curses'])}\n\n_{random.choice(EDGY_JOKES)}_",
        parse_mode="Markdown"
    )
...
async def judgment_day(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"⚡ **СУДНИЙ ДЕНЬ: {effect.upper()}** ⚡\n"

    if effect == "усереднення":
        avg = round(sum(u["weight"] for u iimport os
import random
import threading
import math
import pymongo
...
def main():
    ...
    app_tg = Application.builder().token(os.environ.get("BOT_TOKEN")).build()
...
if __name__ == "__main__":
    main()n users) / len(users), 2)
        ...
        msg += "🛡️ Благословення забрані богами."
...
def main():
    ...
    aapp_tg.add_handler(CommandHandler("start", start))
